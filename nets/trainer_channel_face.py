...
from nets.hsc import latent_compress, feature_compress_channel as feature_compress
...
class Trainer(nn.Module):
    ...
    def init_stylegan(self, config):
        ...
        StyleGAN = Generator(1024, 512, 8) # 图像尺寸，隐空间维度，z到w的线性层层数
        return StyleGAN
    ...
    def update_latent(self, latent=None, n_iter=0):
        ...
        latent = latent.view(batch_size, 9, 32, 32)
    ...
    def compute_compressed_loss_stylegan2(self, w=None, img=None, noise=None, real_img=None, compressed_stage=None):
        ...
        w_recon = w_recon.view(batch_size, 9, 32, 32)
    ...
    def compressed_test_w(self, w=None, img=None, noise=None, zero_noise_input=True, return_latent=False, training_mode=False, compressed_stage=None):        
        if 'n_iter' not in self.__dict__.keys():
            self.n_iter = 1e5
        set_entropy_coder("ans")
        self.latent_compressor.update(force=True)
        self.feature_compressor.update(force=True)
        out = self.get_compressed_image_w(w=w, img=img, noise=noise, training_mode=training_mode, compressed_stage=compressed_stage)
        x_1_recon, x_1, w_recon, w_delta, n_1, fea_1 = out[:6]
        output = [x_1, x_1_recon]
        if return_latent:
            output += [w_recon, fea_1]
    ...
    def get_compressed_image_w(self, w=None, img=None, noise=None, zero_noise_input=True, return_latent=False, training_mode=False, compressed_stage=None):        
        x_1, n_1 = img, noise
        if x_1 is None:
            x_1, _ = self.StyleGAN([w], input_is_latent=True, noise = n_1)
        ...
        if 'use_fs_encoder' in self.config and self.config['use_fs_encoder']:
            ...
            if compressed_stage != 4:
                ...
                bpp = (len(w_recon_encode["strings"][0][0]) + len(fea_encode["strings"][0][0])) * 8.0 / 1024 / 1024
                latent_bpp = len(w_recon_encode["strings"][0][0]) * 8.0 / 1024 / 1024
                feature_bpp = len(fea_encode["strings"][0][0]) * 8.0 / 1024 / 1024
                print(latent_bpp, feature_bpp, bpp)
                self.test_bpp += bpp
                self.test_num += 1
        ...
        # generate image
        # x_1_recon, fea_recon = self.StyleGAN([w_recon], input_is_latent=True, return_features=True, features_in=features, feature_scale=min(1.0, 0.0001*self.n_iter)) # 得到重建人脸和插入的feature
        x_1_recon, fea_recon = self.StyleGAN([w_recon], input_is_latent=True, return_features=True, features_in=None, feature_scale=1) # 得到重建人脸和插入的feature
        fea_recon = fea_recon[k].detach() 
        return [x_1_recon, x_1[:,:3,:,:], w_recon, w_delta, n_1, fea, fea_recon]
    ...
    def get_compressed_image(self, w=None, img=None, noise=None, zero_noise_input=True, training_mode=True, compressed_stage=None):
        ...
        # x_1_recon, fea_recon = self.StyleGAN([self.dlatent_avg], input_is_latent=True, return_features=True, features_in=features, feature_scale=1) # 只用平均值去重建
        fea_recon = fea_recon[k].detach() 
        return [x_1_recon, x_1[:,:3,:,:], w_recon, w_delta, n_1, fea, fea_recon]
